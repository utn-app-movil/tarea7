...
object ApiClient {
    ...
    // Configuración de Retrofit para la API de Dog CEO
    private val dogRetrofit: Retrofit by lazy {
        Retrofit.Builder()
            .baseUrl(DOG_BASE_URL) // Establece la URL base de la API de Dog CEO
            .client(okHttpClient) // Asigna el cliente HTTP configurado
            .addConverterFactory(json.asConverterFactory("application/json".toMediaType())) // Usa el convertidor de Kotlinx Serialization
            .build()
    }
    ...
    // Exposición de la API de Dog CEO configurada con Retrofit
